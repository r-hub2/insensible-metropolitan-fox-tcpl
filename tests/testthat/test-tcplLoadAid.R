#-------------------------------------------------------------------------------
# Covers testing tcplLoadAid with "API" driver
...
httptest::with_mock_dir("ctx", {
  ...
  test_that("tcplLoadAid works by aeid", {
    expect_no_error(dat <- tcplLoadAid(fld = "aeid", val = test_api$aeid))
    expect_true(is.data.table(dat))
    expect_equal(nrow(dat), 1)
    expect_true(all(c("aeid", "aid", "anm") %in% colnames(dat)))
    expect_equal(dat$aid, test_api$aid)
  })
  test_that("tcplLoadAid works by non-id", {
    expect_no_error(dat <- tcplLoadAid(fld = c("intended_target_type", "detection_technology_type"), val = list(assays$intended_target_type[1], assays$detection_technology_type[1])))
    expect_true(is.data.table(dat))
    expect_true(nrow(dat) > 0)
    expect_true(all(c("intended_target_type", "detection_technology_type", "aid", "anm") %in% colnames(dat)))
  ...
  test_that("data not found results in null data.table", {
    expect_no_error(dat <- tcplLoadAid(fld = "aeid", val = 0))
